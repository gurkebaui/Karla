#!/usr/bin/env python3
"""
pretrain_karla.py - Phase 3: Streaming Tool-Use Pretraining
===========================================================
- Nutzt max_steps statt Epochen (perfekt für Streaming/IterableDatasets).
- Graceful Exit und Fallback Checkpointing inklusive.
"""
...
logger = logging.getLogger("PretrainKarla-Streaming")
...
class PretrainConfig:
    max_steps: int = 50000       # NEU: Feste Anzahl an Trainings-Schritten für Streaming
    max_length: int = 512       
    batch_size: int = 2
...
class StreamingTrainer:
    def __init__(self, model, train_loader, val_loader, config):
        ...
        self.val_loader = val_loader # Ist bei uns None, das ist okay!
        self.config = config
        ...
        # NEU: Kein len(train_loader) mehr! Wir nutzen einfach config.max_steps
        ...
        self.global_step, self.consecutive_nans = 0, 0
        
        os.makedirs(os.path.abspath(config.output_dir), exist_ok=True)

    def train(self):
        logger.info("=" * 60)
        logger.info("STARTING PHASE 3: MASSIVE STREAMING PRETRAINING")
        logger.info(f"Target: {self.config.max_steps} Steps")
        ...
        try:
            while self.global_step < self.config.max_steps:
                try:
                    ...
                    # Falls der Stream aus irgendeinem Grund leerläuft, neustarten
                    logger.info("Stream exhausted. Restarting stream...")
                    train_iter = iter(self.train_loader)
                    batch = next(train_iter)
                ...
                if torch.isnan(scaled_loss) or torch.isinf(scaled_loss):
                    ...
                    if self.consecutive_nans >= self.config.nan_patience: 
                        logger.error("Too many NaNs. Stopping.")
                        break
                    self.optimizer.zero_grad()
                    continue
                ...
                if accum_step >= self.config.gradient_accumulation_steps:
                    ...
                    if self.global_step % self.config.log_interval == 0:
                        ...
                        logger.info(f"[Step {self.global_step}/{self.config.max_steps}] Loss: {total_loss/num_batches:.4f} | LR: {self.scheduler.get_last_lr()[0]:.2e} | CTM: {self.model.ctm_scale().item():.4f}{l1_info} | GradNorm: {math.sqrt(grad_norm):.2f}")
                        total_loss, num_batches = 0.0, 0 # Reset averager
    ...
    def _save_checkpoint(self, filename: str):
        ...
        checkpoint = {
            ...
            "training_state": {
                ...
                "epoch": 0, # Wird im Streaming nicht genutzt
            },
            "scales": {
    ...
    def load_checkpoint(self, path: str):
        ...
        # -------------------------------------------------------------
        # SMART FILTER: Ignoriert Parameter, deren Größe sich geändert hat!
        # (Z.B. wenn man von 64 auf 256 Experten hochskaliert)
        # -------------------------------------------------------------
        filtered_state_dict = {}
        for k, v in state_dict.items():
            if k in model_state and v.shape != model_state[k].shape:
                logger.warning(f"Ignoriere '{k}' wegen Größenänderung: Checkpoint {v.shape} -> Modell {model_state[k].shape}")
            else:
        ...
        if missing:
            logger.info(f"Neu initialisierte Module (z.B. skalierte Experten): {len(missing)} Keys.")
        
        try:
            # Versuche Optimizer-States zu laden (klappt oft nicht, wenn Parameter skaliert wurden, was okay ist)
            self.optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
            self.scheduler.load_state_dict(checkpoint["scheduler_state_dict"])
            self.global_step = checkpoint["training_state"].get("global_step", 0)
            logger.info(f"Resumed successfully from step {self.global_step}")
        except Exception as e:
            # Fallback: Wir fangen mit einem frischen Optimizer an (Das CTM-Wissen in den Gewichten bleibt!)
            logger.warning(f"Architektur geändert. Optimizer startet frisch. (Normal bei Skalierung).")
            self.global_step = checkpoint["training_state"].get("global_step", 0)
def main():
    ...
    train_loader, val_loader = create_dataloaders(tokenizer=tokenizer, batch_size=config.batch_size, max_length=config.max_length)

    trainer = StreamingTrainer(model, train_loader, val_loader, config)
